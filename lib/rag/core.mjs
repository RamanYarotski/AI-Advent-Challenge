...
function defaultSourceInputs(root = process.cwd()) {
  return defaultCorpusRoots(root).map((item) => relativeSource(item, root));
}

function sourceInputsFrom(input, root = process.cwd()) {
  if (Array.isArray(input.sourceInputs) && input.sourceInputs.length) {
    return input.sourceInputs.map(String).map((item) => item.trim()).filter(Boolean);
  }
  ...
  if (Array.isArray(input.corpusRoots) && input.corpusRoots.length) {
    return input.corpusRoots.map(String).map((item) => item.trim()).filter(Boolean);
  }
  return defaultSourceInputs(root);
}
...
async function fetchTextDocument(sourceUrl, sourceType = "url") {
  ...
  const title = format === "html" ? titleFromHtml(raw, new URL(sourceUrl).pathname) : path.basename(new URL(sourceUrl).pathname) || sourceUrl;
  return {
    id: sourceUrl,
    source: sourceUrl,
    sourceType,
...
async function fetchGithubDocuments(sourceInput) {
...
export async function loadDocuments(input = {}) {
  ...
  const sourceInputs = sourceInputsFrom(input, root);
  const corpusRoots = sourceInputs
  ...
  for (const corpusRoot of corpusRoots) {
    ...
    const warning = !exists
      ...
      : discovered.length === 0
        ...
        : undefined;
    if (warning) {
      warnings.push(`${relativeSource(corpusRoot, root)}: ${warning}`);
    }
    sourceSummaries.push({
      input: relativeSource(corpusRoot, root),
      type: "local",
      documentCount: discovered.length,
      ...(warning ? { warning } : {}),
    });
  ...
  for (const sourceInput of sourceInputs.filter(isHttpUrl)) {
    try {
      const fetchedDocuments = isGithubUrl(sourceInput)
        ? await fetchGithubDocuments(sourceInput)
        : [await fetchTextDocument(sourceInput, "url")];
      const usableDocuments = fetchedDocuments.filter((document) => document.text.length >= 80);
      sourceSummaries.push({
        input: sourceInput,
        type: isGithubUrl(sourceInput) ? "github" : "url",
...
function meaningfulTokens(text) {
  return tokenize(text).filter((token) => !STOP_WORDS.has(token) && token.length > 2);
}
...
function localAnswer(question, matches) {
  ...
  const citations = matches.slice(0, 3).map((match, index) => {
    const excerpt = normalizeWhitespace(match.text).slice(0, 260);
  ...
  return [
    ...
    normalizeWhitespace(lead.text).slice(0, 700),
    "",
    "Опорные фрагменты:",
    ...citations,
  ].join("\n");
